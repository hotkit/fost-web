...
FSL_TEST_SUITE(response_301);


FSL_TEST_FUNCTION(no_location_config) {
    fostlib::http::server::request req;
    FSL_CHECK_EXCEPTION(fostlib::urlhandler::response_301(
...
FSL_TEST_FUNCTION(with_location_config) {
    ...
    FSL_CHECK_EQ(response.first->headers()["Location"].value(), "http://test.example.com/");
}
